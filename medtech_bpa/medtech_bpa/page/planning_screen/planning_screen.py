...
def get_items_data(from_date, to_date,listview=None):
        ...
        item_detail = frappe.db.sql("select i.item_code,i.item_name, i.item_group, i.stock_uom from `tabItem` i join `tabBOM` b on i.item_code = b.item where b.docstatus = 1 and i.item_group in ({0}) group by i.item_code order by i.item_group, i.item_code".format(fg_item_group_list), as_dict=1,debug=1)
