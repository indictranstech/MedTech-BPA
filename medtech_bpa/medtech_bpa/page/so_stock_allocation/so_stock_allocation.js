// frappe.pages['so_stock_allocation'].on_page_load = function(wrapper) {
// 	var page = frappe.ui.make_app_page({
// 		parent: wrapper,
// 		title: 'Sales Order Stock Allocation',
// 		single_column: true
// 	});
// }

frappe.pages['so_stock_allocation'].on_page_load = function(wrapper) {
	new frappe.so_stock_allocation({
		$wrapper: $(wrapper)
	});
	//frappe.breadcrumbs.add("MedTech-BPA");
}

frappe.so_stock_allocation = Class.extend({
	init: function(opts) {
	...
	fetch_pending_so: function(data) {
		...
		frappe.call({
			method: "medtech_bpa.medtech_bpa.page.so_stock_allocation.so_stock_allocation.get_pending_so",
			args: data,
			callback: function(r) {
				if(!r.exc) {
	...
	save_doc: function() {
		...
		$('.save_doc').on("click", function() {
			...
			if(approved_items == 0) {
				...
				frappe.call({
					method: "medtech_bpa.medtech_bpa.page.so_stock_allocation.so_stock_allocation.save_stock_allocation",
					args: {"data": data},
					async: false,
					callback: function(r) {
						if(!r.exc) {
	...
	submit_doc: function() {
		...
		$('.submit_doc').on("click", function() {
			...
			if(approved_items == 0) {
				...
				var data =  me.get_localstorage_data()
				frappe.call({
					method: "medtech_bpa.medtech_bpa.page.so_stock_allocation.so_stock_allocation.submit_stock_allocation",
					args: {"data": data},
					async: false,
					callback: function(r) {
						if(!r.exc) {
